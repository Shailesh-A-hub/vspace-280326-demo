# Low-Power SpO2 Ratio Engine - Open-Source Pulse Oximeter Signal Processor

## Project Title: Low-Power SpO2 Ratio Engine - Open-Source Pulse Oximeter Signal Processor
  ...
  - - AC Peak Holder x2 - 8-bit register updated when |sample - DC| exceeds current max; resets every 256 clock cycles (~100 gates)
    - - Ratio Unit - Computes R using cross-multiplication and bitshift-based division: R = (AC_red * DC_ir) >> (AC_ir * DC_red); implemented with shift-and-add multipliers (~200 gates)
      - - 16-Entry Calibration LUT - Hardcoded case ROM mapping 4-bit R index to 7-bit SpO2 value (calibrated per Beer-Lambert empirical curve) (~112 gates)
        - - Control FSM - 4-state Moore machine managing sample sequencing, reset timing, and output latching (~80 gates)
         
          - **Total Estimated Gate Count:** ~730 gates
          ...
          - 1) 4-sample averaging instead of 8 - halves the shift register depth and adder complexity without significantly affecting DC accuracy at typical 100Hz PPG sampling rates
            2) 2) Bitshift-only arithmetic - all multiplication and division uses power-of-2 approximations (shift + add), eliminating hardware multiplier blocks which typically cost 200-400 gates alone
               3) 3) 4-bit LUT addressing - reduces the calibration ROM to 16 entries (7-bit wide), sufficient for clinical SpO2 accuracy of +-2% per ISO 80601-2-61
                  ...
                  4) ### Why This Design:
                  5) There is currently no open-source ASIC implementation of a SpO2 ratio engine on the SkyWater Sky130 or IHP 130nm PDK. All commercial pulse oximeter ICs (MAX30102, AFE4490) are proprietary. This design, once taped out, would be the first open-silicon SpO2 processor - directly enabling domestic Indian semiconductor IP in the wearable healthcare space, a priority sector under India's Semicon India Programme.
                 
                  6) ### Gate Budget Breakdown
                  7) - SPI Deserializer - 60 gates (8-bit shift register, channel-select logic)
                     - - DC Extractor x 2 channels - 180 gates (4-tap shift register banks + adder tree + right-shift)
                       - - AC Peak Holder x 2 channels - 100 gates (running max registers + absolute value unit)
                         - - Ratio Computation Unit - 200 gates (shift-and-add cross-multiplier + bitshift divider)
                           - - 16-Entry Calibration LUT - 112 gates (hardcoded case ROM, 7-bit wide)
                             - - Control FSM - 80 gates (4-state Moore machine, sequencing + reset logic)
